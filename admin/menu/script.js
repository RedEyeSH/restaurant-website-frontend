...
document.getElementById('itemForm').addEventListener('submit', async function (e) {
    ...
    itemData.append('category', document.getElementById('category').value);
    ...
    itemData.append('ingredients',tinymce.get('ingredients').getContent());
    itemData.append('allergens', allergensString);
    itemData.append('size', document.getElementById('size').value);
    itemData.append('price', parseFloat(document.getElementById('price').value));
    itemData.append('image', document.getElementById('image').files[0]); // Ensure the image is added to the FormData
    itemData.append('stock', document.getElementById('stock').value);
    itemData.append('visible', document.getElementById('visible').value);
    ...
    // prin the itemdata in pretty format json stringify
    console.log('Item Data:', JSON.stringify(Object.fromEntries(itemData), null, 2)); // Debugging line to check the item data
    ...
    try {
        ...
        for (const item of data) {
            ...
            row.innerHTML = `
                ...
                <td>${item.price}€</td>
                <td>${new Date(item.created_at).toLocaleString('fi')}</td>
        ...
        <div class="modal-body">
          ...
          <div class="card mb-3">
            ...
            <div class="card-body row">
              <div class="col-md-6">
                <p><i class="bi bi-hash me-2"></i><strong>ID:</strong> ${item.id}</p>
                <p><i class="bi bi-tags me-2"></i><strong>Category:</strong> ${item.category}</p>
              </div>
              <div class="col-md-6 text-end">
                <p style="font-size: 1.5rem;"><strong><i class="bi bi-currency-euro me-1"></i>Price:</strong> ${item.price}€</p>
                <div>
          ...
          <div class="card mb-3">
            <div class="card-header bg-secondary text-white">
              <i class="bi bi-image me-2"></i>Preview Image & Description
            </div>
            <div class="card-body text-center">
              <img src="https://10.120.32.59/app${item.image_url}" alt="${item.name}" class="img-fluid rounded" style="max-height: 300px;">
              <p class="mt-4"><i class="bi bi-card-text me-2"></i><strong>Description:</strong> ${item.description}</p>
            </div>
          </div>
          ...
          <div class="card mb-3">
            <div class="card-header bg-info text-white">
              <i class="bi bi-list-check me-2"></i>Contents
            </div>
            <div class="card-body row">
              <div class="col-md-6">
                <p><i class="bi bi-basket me-2"></i><strong>Ingredients:</strong> ${item.ingredients}</p>
                <p><i class="bi bi-exclamation-triangle me-2"></i><strong>Allergens:</strong> ${item.allergens || 'None'}</p>
              </div>
            </div>
          </div>
          ...
          <div class="card mb-3">
            ...
            <div class="card-body row">
              <div class="col-md-6">
                <p><i class="bi bi-calendar-plus me-2"></i><strong>Created At:</strong> ${new Date(item.created_at).toLocaleString('fi')}</p>
                <p><i class="bi bi-person-plus me-2"></i><strong>Added By:</strong> ${item.added_by ? await fetchUserName(item.added_by) : 'Null'}</p>
              </div>
              <div class="col-md-6 text-end">
                <p><i class="bi bi-calendar-check me-2"></i><strong>Updated At:</strong> ${new Date(item.updated_at).toLocaleString('fi')}</p>
                <p><i class="bi bi-person-check me-2"></i><strong>Updated By:</strong> ${item.updated_by ? await fetchUserName(item.updated_by) : 'Not yet updated'}</p>
              </div>
            </div>
          </div>
...
async function viewMealDetails(id) {
    try {
        ...
        // Fetch names and visibility for each item dynamically using the meal's item_ids
        const itemIds = Object.values(meal.item_ids);
        const itemsData = await Promise.all(itemIds.map(itemId => fetchItemData(itemId)));

        // Dynamically generate the HTML for related items
        let relatedItemsHTML = '';
        
        // Only display items that exist in the item_ids object
        itemIds.forEach((itemId, index) => {
            const item = itemsData[index];
            
            // Skip if the item is not available
            if (item.name !== 'Not available') {
                const itemName = item.name;
                const itemVisibility = item.visible;
                const visibilityBadgeClass = itemVisibility.toLowerCase() === 'yes' ? 'bg-success' : 'bg-danger';
                const visibilityText = itemVisibility.toLowerCase() === 'yes' ? 'Visible' : 'Not Visible';

                relatedItemsHTML += `
                    <p>${itemName} <span class="badge ${visibilityBadgeClass}">${visibilityText}</span></p>
                `;
            }
        });
        ...
        viewMealDetailsDiv.innerHTML = `
            <div class="modal-header">
                <h5 class="modal-title text-center w-100">
                    <i class="bi bi-restaurant me-2"></i>${meal.name}
                </h5>
            </div>
            <div class="modal-body">
                ...
                <div class="card mb-3">
                    <div class="card-header bg-primary text-white">
                        <i class="bi bi-info-circle me-2"></i>Meal Information
                    </div>
                    <div class="card-body row">
                        <div class="col-md-6">
                            <p><i class="bi bi-hash me-2"></i><strong>ID:</strong> ${meal.id}</p>
                        </div>
                        <div class="col-md-6 text-end">
                            <p style="font-size: 1.5rem;"><strong><i class="bi bi-currency-euro me-1"></i>Price:</strong> ${meal.price}€</p>
                            <div>
                ...
                <div class="card mb-3">
                    <div class="card-header bg-secondary text-white">
                        <i class="bi bi-image me-2"></i>Meal Preview
                    </div>
                    <div class="card-body text-center">
                        <img src="https://10.120.32.59/app${meal.image_url}" alt="${meal.name}" class="img-fluid rounded" style="max-height: 300px;">
                        <p class="mt-4"><i class="bi bi-card-text me-2"></i><strong>Description:</strong> ${meal.description}</p>
                    </div>
                </div>
                ...
                <div class="card mb-3">
                    <div class="card-header bg-info text-white">
                        <i class="bi bi-list-check me-2"></i>Related Items
                    </div>
                    <div class="card-body">
                        ${relatedItemsHTML}
                    </div>
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body row">
                        <div class="col-md-6">
                            <p><i class="bi bi-calendar-plus me-2"></i><strong>Created At:</strong> ${new Date(meal.created_at).toLocaleString('fi')}</p>
                            <p><i class="bi bi-person-plus me-2"></i><strong>Added By:</strong> ${meal.added_by ? await fetchUserName(meal.added_by) : 'Null'}</p>
                        </div>
                        <div class="col-md-6 text-end">
                            <p><i class="bi bi-calendar-check me-2"></i><strong>Updated At:</strong> ${new Date(meal.updated_at).toLocaleString('fi')}</p>
                            <p><i class="bi bi-person-check me-2"></i><strong>Updated By:</strong> ${meal.updated_by ? await fetchUserName(meal.updated_by) : 'Not yet updated'}</p>
                        </div>
                    </div>
                </div>
...
async function editItem(id) {
    ...
    try {
        ...
        tinymce.get('editIngredients').setContent(item.ingredients);
        ...
        editForm.onsubmit = async function(e) {
            ...
            // Ensure TinyMCE editor instance is initialized
            ...
            formData.append('ingredients', editIngredientsEditor.getContent());
...
// Add a confirmation dialog before deleting a meal
...
// Function to handle delete button click for meals
async function deleteMeal(id) {
    try {
        ...
        if (response.ok) {
            showToast('Meal deleted successfully!'); // Show success toast
            fetchMeals(); // Refresh the meals table
        } else {
            showErrorModal('Error deleting meal.');
        }
    } catch (error) {
