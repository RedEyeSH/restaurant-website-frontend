    ...
    try {
        ...
        <div class="container">
          ...
          <div class="card mb-3">
            ...
            <div class="card-body">
              <p><i class="bi bi-hash me-2"></i><strong>Reservation ID:</strong> ${reservation.reservation_id}</p>
              <p><i class="bi bi-calendar me-2"></i><strong>Date:</strong> ${formattedDate}</p>
              <p><i class="bi bi-clock me-2"></i><strong>Time:</strong> ${reservation.time}</p>
              <p><i class="bi bi-people me-2"></i><strong>Guest Count:</strong> ${reservation.guest_count}</p>
              <p><i class="bi bi-grid-3x3-gap me-2"></i><strong>Allocated Table Number:</strong> ${reservation.allocated_tables}</p>
            </div>
          </div>
          ...
          <div class="card mb-3">
            ...
            <div class="card-body">
              <p><i class="bi bi-person me-2"></i><strong>Name:</strong> ${reservation.name}</p>
              <p><i class="bi bi-telephone me-2"></i><strong>Phone:</strong> ${reservation.phone}</p>
              <p><i class="bi bi-envelope me-2"></i><strong>Email:</strong> ${reservation.email}</p>
              <p><i class="bi bi-chat-left-text me-2"></i><strong>Notes:</strong> ${reservation.notes || 'None'}</p>
            </div>
          </div>
          ...
          <div class="card mb-3">
            <div class="card-header bg-dark text-white">
              <i class="bi bi-info-circle me-2"></i>Audit Information
            </div>
            <div class="card-body">
              <p><i class="bi bi-calendar-plus me-2"></i><strong>Created At:</strong> ${new Date(reservation.created_at).toLocaleString('fi')}</p>
              <p><i class="bi bi-calendar-check me-2"></i><strong>Updated At:</strong> ${new Date(reservation.updated_at).toLocaleString('fi')}</p>
              <p><i class="bi bi-person-check me-2"></i><strong>Updated By:</strong> ${reservation.updated_by ? await fetchUserName(reservation.updated_by) : 'Not yet updated'}</p>
            </div>
          </div>
...
document.getElementById('editReservationForm').addEventListener('submit', async function (event) {
    ...
    document.getElementById('editReservationLoading').style.display = 'none'; // Hide loading spinner
...
document.getElementById('editTableForm').addEventListener('submit', async function (event) {
    ...
    document.getElementById('editReservationTableLoading').style.display = 'block'; // Show loading spinner
    ...
    try {
        ...
        fetchAvailableDays(); // Refresh the available days list
        fetchReservations(); // Refresh the reservations list
        document.getElementById('editReservationTableLoading').style.display = 'none'; // Hide loading spinner
    } catch (error) {
